...
const promptDepartmentName = new startInquirer(typeInquire[2], 'dep_name', questions.allEmployDepartment, departmentNameArray);
...
const empByDepTable = new querySQL(query, userResponse.department_Name);
...
//Create an instance of startInquirer and then deliver it a prompt for inquirer.
    const rolePrompt = new startInquirer(typeInquire[2], 'role_Title', questions.viewAllEmpByRole, compRoles);
...
employee.manager_id, department.name FROM employee 
...
connection.query(queryMultipleEmpCheck, [empFirstName, empLastName], function (error, result) {
 if (result.length > 1) {
            ...
            const which_employee_to_Delete = new startInquirer(typeInquire[2], 'employee_delete', questions.deleteEmployee1, multipleName);
...
function updateEmpRole(employeeID, RolesArray) {
    ...
    const empNewRole = new startInquirer(typeInquire[2], 'employee_role', questions.updateRole, RolesArray);
